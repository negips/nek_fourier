...
!      call copy(t,vz,ntot1)
      ...
      call rzero(prp,ntot2)
      call rzero(prp(1,2),ntot2)
      ...
      if (mod(istep,iostep).eq.0) then
         call outpost(vxp,vyp,vzp,prp,vzp,'ptr')
         call outpost(vxp(1,2),vyp(1,2),vzp(1,2),
     $                prp(1,2),vzp(1,2),'pti')
       endif  

      if (istep.eq.-1) then
         call outpost(tmp1,tmp2,tmp3,tmp4,tmp3,'tmp')
         call outpost(tmp5,tmp6,tmp7,tmp8,tmp7,'tmp')

         call outpost(bfxp,bfyp,bfzp,prp,bfzp,'bfp')
...
!      zscale = ZLEN/(zmax-zmin)
      ...
      do j=1,nelv
      do i=1,2**ldim
         xc(i,j) = xc(i,j)*2.0*pi*alphai
         yc(i,j) = yc(i,j)*2.0*pi*alphai
      enddo
      enddo
