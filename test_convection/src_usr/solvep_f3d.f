...
!       For both real/complex together            
        if (igeom.gt.1) then
          jp = jp0+1
...
!       Need to Extrapolate both pressures at the same time    
        call extrapprp (prextr_f3d(1,1))        ! real pressure
        jp = jp + 1
        call extrapprp (prextr_f3d(1,2))        ! imaginary pressure
        jp = jp - 1
...
!             up    := pressure correction req'd to impose div U = 0
!
!
...
!     Remove constant pressure only when kz=0.      
...
!     Map X from mesh M2 to mesh M1 (Y)
!     Includes mass matrix multiplication        
